...
from utils.hbase_client import THBaseService
from utils.hbase_client.ttypes import TTransport, TColumnValue, TGet, TPut, TDelete, TScan
...
class HBaseClient2(object):
    ...
    def h_get(self, table: str, row: str):
        ...
        return {column.qualifier.decode(): column.value.decode() for column in
                result.columnValues}

    def h_put(self, table: str, row: str, column_family: str, **data_items):
        values = [TColumnValue(family=column_family.encode(),
                               qualifier=k.encode(),
                               value=str(v).encode())
    ...
    def h_scan(self,
        ...
        t_scan = TScan(**{k: v.encode() if type(v) == str else str(v).encode()
        ...
        while row_list:
            for r in row_list:
                dict_data = {c.qualifier.decode(): c.value.decode() for c in r.columnValues}
...
if __name__ == '__main__':
    HBASE_INFO_SERVERS = [("192.168.120.70", 9090)]
